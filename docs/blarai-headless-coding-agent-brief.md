...
**2026-07-05 — the §6 [VERIFY] landmines are resolved: both upstream issues CLOSED without fixes; on Lunar Lake GPU memory never auto-releases; watch model_server PR #4332 instead.**
A trusted-source research pass (BlarAI fleet-maturation plan, `blarai/docs/research/fleet-maturation-program-plan-2026-07.md` §8, Vikunja #740) settled the two upstream questions this brief flags as [VERIFY]: **openvino #11978** (a first-class GenAI unload API) was closed in 2022 — destroy-the-object + `gc.collect()` remains the only unload path, exactly as §6.1 assumed. **openvino #33896** was closed 2026-06-29 for *inactivity*, root cause never identified — and the load-bearing nuance is that the garbled-output-after-idle-release behavior belongs to **Panther Lake**; on **Lunar Lake (this box) GPU memory simply never auto-releases on idle**, which spares us the corruption bug but confirms explicit full-teardown + fresh-reload is the only free-the-GPU lever. Net: the swap design is *confirmed correct*, not merely cautious — do not relitigate it. The live thread that matters now is **model_server issue #4141 + PR #4332** (native `idle_unload_timeout_seconds`, an `UNLOADED` state, transparent lazy reload — GPU-verified per the PR): when it merges into a tagged release it is the sanctioned replacement for the stop/restart half of the swap. Tracked with a monthly release check at Vikunja **#741** (the supportive upstream post with our swap data is deferred to a future session by LA instruction). Two related facts from the same pass: no persistent/cross-swap KV cache exists or is roadmapped (prefix caching pays only *within* one residency — keep batching the whole plan into one swap), and OVMS 2026.0's compiled-model cache is **absent from our launch** (`start-llm.ps1:223` passes `--cache_size` but no `--cache_dir`) — adding it is queued in the M2 hardening sweep to cut the 30–90 s swap-in compile.
