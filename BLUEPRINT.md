...
3. **≥19 GB available** before loading the 30B (the script checks this for you; the exact floor is `$needGB` in `start-llm.ps1`, held in lockstep with BlarAI's `[fleet_dispatch].swap_min_free_gb` — read those rather than this line if they ever disagree).
